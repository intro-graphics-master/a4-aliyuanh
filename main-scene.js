  ...
  constructor()
    {                  // constructor(): Scenes begin by populating initial values like the Shapes and Materials they'll need.
     ...
     // this.shapes.ball_repeat.arrays.texture_coord.forEach( coord => coord);
      ...
      const fun_shader = new defs.Funny_Shader();
      ...
      this.materials = { plastic: new Material( phong_shader, 
                   ...
                   earth: new Material( texture_shader_2,    
                          ...
                          funBoi : new Material(fun_shader, {ambient:0,diffusivity:.8, specularity:.5, smoothness:10, color: Color.of(1,1,1,1)}),
                          blackHole: new Material(black_hole_shader, {ambient: 1, diffusivity: .5, specularity: .5, color: Color.of(.7,.3,.3,1)}),
                          bestSun: new Material(sun_shader, {ambient:1, diffusivity: .5, specularity: .5, color: Color.of(.5,.5,.5,.5)}),
  ...
  display( context, program_state )
    {                                                // display():  Called once per frame of animation.  For each shape that you want to
      ...
      **********************************/         
      ...
      program_state.lights = [ new Light( Vec.of( 0,0,0,1 ), sun_color, 10000000**(sun_size)) ];
      ...
      this.shapes.ball_6.draw(context,program_state,model_transform,this.materials.bestSun.override(sun_color));
      //first planet
      if(this.lights_on){
      ...
      model_transform = model_transform.post_multiply(Mat4.translation([-3,0,0]));
      model_transform = model_transform.post_multiply(Mat4.rotation(-Math.PI/2,[0,1,0]));
      ...
      this.shapes.ball_2.draw(context,program_state,model_transform,this.materials.m2);
            var camThing = model_transform.copy();
       camThing =camThing.post_multiply(Mat4.translation([-3,0,0]));
      camThing = camThing.post_multiply(Mat4.rotation(-Math.PI/2,[0,1,0]));
      this.camera_teleporter.cameras.push( Mat4.inverse( camThing.copy()));
      //moon number 1!!!!!
      model_transform = model_transform.post_multiply(Mat4.rotation(t/3,[0,1,0]));
      model_transform = model_transform.post_multiply(Mat4.translation([2,0,0]));
      model_transform = model_transform.post_multiply(Mat4.scale([.75,.75,.75]));
      //this.shapes.ball_6.draw(context, program_state, model_transform, this.materials.funBoi);
      this.shapes.ball_6.draw(context, program_state, model_transform, this.materials.blackHole);
      var camThing = model_transform.copy();
      camThing = camThing.post_multiply(Mat4.scale([1/.75,1/.75,1/.75]));
      camThing = camThing.post_multiply(Mat4.translation([0,0,2.5]));
      this.camera_teleporter.cameras.push( Mat4.inverse( camThing.copy()));
      //third planet -- earth! 
      model_transform = origin_system.copy();
      ...
      var camThing = model_transform.copy();
      camThing = camThing.post_multiply(Mat4.rotation(-t/3, [0,1,0]));
      camThing = camThing.post_multiply(Mat4.translation([0,0,1.5]));
      this.camera_teleporter.cameras.push( Mat4.inverse( camThing.copy()));
      //model_transform = model_transform.post_multiply(Mat4.rotation(t/3, [0,1,0]));
      //model_transform = model_transform.post_multiply(Mat4.translation([0,0,-1.5]));
      //moon 2!!!!!!!!
  ...
  vertex_glsl_code()           // ********* VERTEX SHADER *********
    {
      ...
      return this.shared_glsl_code() + `
        ...
        {   // The vertex's final resting place (in NDCS).
          ...
          float sin_value = mod( 5.*animation_time + f_tex_coord.y*30., radians(360.) );
          float rate = 0.2* sin( sin_value ) + 1.2;
          vec3 pose = vec3( position.x * rate, position.y * rate, position.z * rate);
          gl_Position = projection_camera_model_transform * vec4( pose, 1.0);        }`;
    }
  fragment_glsl_code()           // ********* FRAGMENT SHADER *********
    { 
